    ...
    BigTree.prototype = {
        ...
        _registerPlugins: function() {
            ...
            $.each(plugins, $.proxy(function(i, p){
                var id = p.id !== undef ? p.id : i;
                if (p.template) {
                    p.id = id;
                    $(p.template).attr('data-plugin-id', id).appendTo(p.placement == 'tail' ? tail : head);
                }    
            }, this));
        ...
        _renderRange: function(data, start, end) {
            ...
            this._suspendPlugins(this.visible());
            this.editor.detach();

            this._visible = range;
            this._attachPlugins(range);
        ...
        _attachPlugins: function(datas) {
            ...
            for (i = 0; i < dsize; i++) {
                if (datas[i].plugins === undef) {
                    datas[i].plugins = {};
                    for (j = 0; j < psize; j++) {
                        plugin = $.extend({}, plugins[j]);
                        plugin.onInitialize(this, datas[i]);
                        datas[i].plugins[plugin.id] = plugin;
                    }
                }
            }
        },
